...
class SqlAlchemyMarketplaceSyncRepository:
    def __init__(self, session: Session) -> None:
        self.session = session

    def upsert_shop(self, config: ConfiguredMarketplaceShop) -> int:
        ...
        if shop is None:
            shop = Shop(
                ...
                platform_shop_id=config.platform_shop_id,
                app_key=config.app_key.get_secret_value(),
                is_active=1,
            )
            self.session.add(shop)
        else:
            if shop.platform != config.platform:
                raise ValueError(
                    f"店铺代号 {config.shop_code} 已属于 {shop.platform}，拒绝覆盖"
                )
            shop.shop_name = config.shop_name
            shop.platform_shop_id = config.platform_shop_id
    ...
    def get_cursor_end(self, shop_id: int, sync_scope: str) -> int | None:
        cursor = self.session.execute(
            select(PlatformSyncCursor).where(
                PlatformSyncCursor.shop_id == shop_id,
                PlatformSyncCursor.sync_scope == sync_scope,
            )
        ).scalar_one_or_none()
    ...
    def upsert_refund(
        ...
        shop_id: int,
        refund: NormalizedMarketplaceRefund,
    ) -> bool:
        ...
        for normalized_item in refund.items:
            item = self.session.execute(
                select(AfterSalesItem).where(
                    ...
                    AfterSalesItem.sku_code == normalized_item.sku_code,
                )
            ).scalar_one_or_none()
            if item is None:
                self.session.add(
                    AfterSalesItem(
                        ...
                        applied_quantity=normalized_item.applied_quantity,
                        inspected_quantity=0,
                        item_status=ItemStatus.NORMAL,
                    )
                )
            else:
                item.applied_quantity = normalized_item.applied_quantity
        return created
