...
拼多多订单在旧管理系统中按“延迟导入订单至系统时间”配置留出下单缓冲期，当前线上值为 5 分钟；订单满该时长后且仍处于待发货、未退款状态，才会被旧系统导入。买家在导入前完成仅退款时，ERP 不会生成客户档案和归属业务员。售后工作台对“拼多多 + 未发货 + 仅退款 + 平台已明确退款成功 + ERP 客户查询为空”的记录缓存为 `not_required`，页面显示“快速退款未入 ERP”，不再混入异常“未匹配”，后台周期日志也单独输出 `not_required` 数量；已发货、退货退款、退款未完成或 ERP 查询暂时不可用的记录仍保持原有严格核对和告警口径。
