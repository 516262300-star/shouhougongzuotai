...
2026-09-09 起，有稳定订单号/售后号的未知类型、字段映射错误和单笔 `45001` 错误写入 `marketplace_sync_issues` 后继续同步正常记录；异常台账和窗口游标在同一事务提交，不静默丢单。每店每轮最多重查 20 笔到期异常，退避 1/2/4/8/16/24 小时，成功后解除隔离。有未解决异常仍报告同步异常，并沿用资金流程的同步成功保护；网络、鉴权、无有效标识、列表结构错误或数据库故障仍停止当前窗口。升级需要迁移 `20260909_0023` 并重启 Web/后台，详见[拼多多特殊售后与异常恢复](docs/pdd-non-refund-sync-20260909.md)。
