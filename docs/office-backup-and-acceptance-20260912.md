...
正式运行机已改为 `192.168.2.46`，旧机停写并禁用业务自启动。最终停写快照完成于15:26，24张表、schema `20260919_0029`、逐表数量及CHECK TABLE、发送JSONL/SQLite、配置、三套实际代码和3458项历史审计均已核验恢复。新机16:09启用业务；未清空资金或发群幂等记录。

新机首份日常备份 `daily-20260923-160925` 通过24张表、10项运行文件及代码包核验。开发机原有异机拉取任务仅更新主机/已验证的known_hosts；触发器、运行账户、隐藏启动方式不变。16:32真实拉取成功，任务结果0。下文09-12的21张表验收属于历史记录，不表示当前schema只有21张表。

新机每日04:30的SYSTEM任务已启用，并于17:03实际触发验收：正常等待Worker退出后，17:07完成 `daily-20260923-170738`，任务结果0、无错误，维护停止标记清除，后台由守护恢复。开发机17:08拉取该新备份成功，24张表、10项文件、schema及代码包校验通过，任务结果0。

正式后台启用后的整机重启验收尚未执行；不把系统服务恢复或手动启动当作业务自动恢复验收。迁移过程、当前限制和回退要求见[新机迁移说明](office-machine-migration-20260922.md)。
...
- 随后已完成整机重启验收：等待 Worker 完整进程树安全退出，新增重启前备份并验证 21 张表相关文件，然后执行 Windows 正常重启（未强制关闭应用）。实际开机时间 15:35:02；用户登录 Windows/企微后，SSH、MySQL 自动服务恢复，Web、Worker 与企微同在新的 Session 1，守护任务 LastTaskResult=0。没有人工启动 Web/Worker 来冒充自动恢复。
- 重启后首个业务周期 15:35:53 开始、15:38:40 完成，`ok=true`，模块 1、2、3 healthy，企微通知阶段 completed/error=null，待发和运行中均为 0，无发送阻塞，停止标记已由正常启动流程清除。此周期没有新待发消息，没有为验收重复发送；真实发送证据仍为前述两条任务。
- 重启前备份已另存正式机 `backups/pre-reboot-20260912-153433`，并复制到开发机受保护备份目录，逐文件、账本、监控库与代码包校验通过。每日备份任务重启后仍启用，下一次为 2026-09-13 04:30。真正关闭开发机的物理实验尚未执行。
