...
- 左侧“运行监控”页面每 15 秒只读刷新模块 1/3 常驻进程、最近完整周期、各处理阶段和当前启用范围内的企微拦截任务队列；页面不会触发退款、发送消息或控制企业微信，异常时仍按对应后台运行器恢复流程处理；
...
- `GET /api/v1/monitor/status`：只读返回常驻运行器 PID/周期新鲜度、模块 1/3 阶段结果、企微拦截任务状态汇总和关键执行开关；不执行任何外部动作。
