...
天猫平台已成功的退货退款另有严格的ERP补单闭环：同一父订单的一笔或多笔退款必须与全部平台子单、同一退货运单、ERP退款详情、唯一RC/TH商品及可选税点行、唯一原收款、逐笔SK退款流水和累计应收完整对应；任一缺失或多余即不写。每个Worker周期最多新补一笔，下一笔重新核验余额；已存在流水只对账，超时或结果不明不重发同一笔。该分支不会再次调用平台退款，完整边界见[天猫模块1：暂存认领与ERP补单](docs/tmall-module1-return-claim.md#2026-09-23模块2退货退款成功后的分组erp补单)。
