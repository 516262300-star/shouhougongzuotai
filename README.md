...
2026-09-24 新增[1688主店发货20小时无物流提醒适配](docs/1688-shipment-reminder-20260924.md)：本地测试与正式只读预演通过，待正式切换及验收；仅ERP原销售业务员提醒，不新增平台退款和模块1/2/3资金权限。

2026-09-24 16:29 已正式开启[抖音四店发货20小时无物流提醒](docs/douyin-shipment-enabled-20260924.md)，沿用每5分钟独立巡检，向ERP原销售业务员发布待办；四店页面均显示已开启。本次仅启用提醒，保留正式机现有退款和模块1/2/3配置。此前的[提醒与模块3适配记录](docs/douyin-reminder-module3-20260924.md)是首次代码交付快照，不代表当前正式开关。

2026-09-19 新增[拼多多、天猫发货20小时无物流提醒](docs/shipment-no-trace-reminder.md)：覆盖普通发货订单，按原销售单业务员发布ERP人工待办，独立巡检并持久化防重复记录。

办公室常开电脑与开发机分离的部署流程，见[正式运行机部署说明](docs/office-production-deployment.md)。当前提供受限部署连接工具，业务迁移与接管需另行完成。

> 2026-09-11 天猫模块1退回平账状态纠正：现有后台已运行退货归属和既有退款流水核账，功能页改为“核账已开·认领补单未接入”。不是新增自动认领或ERP资金写入，详见[退回核账状态说明](docs/tmall-return-accounting-20260911.md)。

> 2026-09-11 天猫模块3：已补齐复用现有ERP管理列表/详情、与拼多多相同补单入口及事后核账的适配，不再以新增服务器接口为前提；当前没有确认未发货的真实验收样本，尚未开启生产补单。进度与限制见[现有ERP复用说明](docs/tmall-module3-existing-erp-20260911.md)。

> 2026-09-11 天猫模块1/2有限发布：按当前可核验授权先开放第1、2、4、5店的独立单订单单子单单包裹退款；第3店当前子账号退款查询权限报错，第6店未配置退款授权，均不纳入自动退款。模块3继续关闭，复杂包裹仍须人工核验；发布与验收结果见[有限开放记录](docs/tmall-limited-rollout-20260911.md)。

> 2026-09-10 审计修复分支：请先阅读[资金、退款与发送安全修复说明](docs/audit-safety-fixes-20260910.md)。天猫未适配的整包裹自动退款已失败关闭；ERP明细匹配不再冒充质检通过，桌面发送不再凭画面变化确认成功。需要新数据库迁移，尚未部署或验收，普通在途按已确认规则在通知成功后可先退款，派件/签收及其他安全阻断继续保留。

面向利德仕多平台、多店铺的售后中台。当前仓库已完成 Phase 1 工程骨架、全量数据库初始化、拼多多七店同步、天猫六店售后同步及模块 1/2/3 接入、淘宝/1688/京东/抖音可配置多店只读售后同步，以及模块 1–4 的核心流程。

## 当前边界

- 2026-09-08 模块1闭环新增[逐单退款证据闸门](docs/module1-closure-guard-20260908.md)：退货匹配、客户余额为零只是候选，必须核对平台退款成功及对应 ERP 退款流水才会标记完成；缺证据保留“待核验退款流水”，不自动认领或重复补单。现有逐单流水适配器仅支持拼多多，其他平台不借用拼多多流水放行；历史已完成记录不在本次批量重写。
- 已实现：配置加载、MySQL 连接池、Alembic 迁移、健康检查、Docker Compose、本地拼多多联调、拼多多七店与天猫六店售后增量同步、淘宝/1688/京东/抖音官方 API 只读售后增量同步、模块 1 在途拦截队列与快递 100 退款闸门、模块 2 ERP 客户退货单/退货暂存核对、仓库验货、明细一致后自动退款与异常转人工、模块 1/2/3 常驻后台运行器、模块 3 未发货 ERP 补开退款单与异常待办、模块 4 售后原因自动分类与型号归因看板、模块 5 ERP 退货报废只读同步与损失核定、已出包判定队列、企微机器人通知，以及受独立写开关和店铺白名单保护的拼多多、天猫退款动作。
...
- ERP 未提供独立 API；模块 3 已通过受双重写开关保护的管理系统网页适配器处理“未发货、已退款、有订单但未开退款单”。已出包锁单仍保留人工回填 CLI。模块 2 只读查询 ERP 客户退货单和退货暂存列表，不在 ERP 内新建或认领退货单；实收明细完全一致且对应平台独立退款开关开启时调用拼多多或天猫退款，明细不一致时冻结退款并转人工。
- 所有外部写入默认关闭。企微、拼多多、天猫、ERP 分别受 `QYWX_WRITE_ENABLED`、`PDD_WRITE_ENABLED`、`TMALL_WRITE_ENABLED`、`ERP_WRITE_ENABLED` 及对应功能开关保护。
...
| `TMALL_SHOP_1_REFUND_SESSION_KEY` … `TMALL_SHOP_6_REFUND_SESSION_KEY` | 每店具有退款权限的子账号 SessionKey；适家当前留空 | 无 |
| `TMALL_SHOP_1_CODE` … `TMALL_SHOP_6_CODE` | 天猫六店的本地稳定代号 | `tmall-shop-01` … `tmall-shop-06` |
| `TMALL_SYNC_ENABLED` | 将天猫售后同步接入常驻后台周期 | `false` |
| `TMALL_WRITE_ENABLED` | 天猫平台写操作总开关 | `false` |
| `TMALL_MODULE123_TRIAL_ENABLED` | 将天猫新售后接入模块 1/2/3；变量名为兼容旧配置保留 | `false` |
...
2026-09-10 人工待办统一简化：在途、退货认领/平账、同包裹、验收/申诉、未发货退款通知均去掉模块编号、重复主订单号和冗长商品明细，保留异常原因及处理要求。同包裹全部关联订单去重保留，完整明细、原始报错及内部编号留在审计载荷。新生成和旧排队任务共用格式化规则，兼容旧标识查重，不重发已发布待办；退款后申诉保留独立身份，退款、人工锁和触发条件不变。模板、验证及加载边界见[人工待办文案简化说明](docs/manual-todo-concise-20260910.md)。
...
拼多多增量列表与售后详情的 `after_sales_type` 编码不同：同步器以列表的 2（仅退款）、3（退货退款）、4（换货）、5（补寄）、6（维修）为准；仅当列表缺失该字段时，才按详情的 1/2/3/4/5 编码回退。补寄、维修仅记录，不进入自动拦截、退款或 ERP 退款补单；状态 10/4 不作为这两类售后的退款成功依据，资金状态显示“不涉及退款”。

2026-09-09 起，有稳定订单号/售后号的未知类型、字段映射错误和单笔 `45001` 错误写入 `marketplace_sync_issues` 后继续同步正常记录；异常台账和窗口游标在同一事务提交，不静默丢单。每店每轮最多重查 20 笔到期异常，退避 1/2/4/8/16/24 小时，成功后解除隔离。同日补充：正常窗口已提交但仍有隔离单时报告 warning，只阻止同店同售后号/平台订单号关联动作；同店其他正常订单及其他正常店铺继续自动化。拼多多按最后更新时间返回旧售后；本地未入库、列表与详情均为已完成仅退款且创建已满365天时，系统保留处置证据后自动归档，不再查询可能过期的订单详情。近期单、退货退款、字段不一致或本地已有记录不适用该规则。网络、鉴权、无有效标识、列表结构或数据库故障仍阻止该店当轮群拦截与平台退款，不连带正常店铺。异常过滤覆盖入队、发送/退款选择及执行前复核，不删除异常或放宽原物流/资金条件。数据库需迁移 `20260909_0023`，本次隔离范围修复无新增迁移，详见[拼多多特殊售后与异常恢复](docs/pdd-non-refund-sync-20260909.md)。
...
& .\scripts\migrate-legacy-tmall-refund-credentials.ps1 -LegacyRoot "D:\desktop\codex\daima\leedis-main-b0dce78834fdb46e564599d012d1c61574bdab2f" -EnableModules
.\.venv\Scripts\alembic.exe upgrade head
.\.venv\Scripts\tmall-check-shops.exe
& .\scripts\module1-worker.ps1 -Action Start
```

需要回退时先停止后台，将 `.runtime/env-before-tmall-refund-credentials-*.bak` 的确认版本恢复为仓库根目录 `.env`，执行 `.\.venv\Scripts\alembic.exe downgrade 20260903_0018`，再切回不含天猫退款动作的代码后启动。降级会删除尚存在的两类天猫退款动作记录，执行前必须先导出动作审计并人工确认。
...
单店失败只回滚该店当前窗口，其他店继续；重新执行会从该店最后成功游标向前重叠 5 分钟幂等续传。若退款记录仍存在、但原交易已经无权查看或被平台归档，系统保留退款数据并将交易详情降级为空，不会让这一笔旧交易阻断整店同步。`TMALL_SYNC_ENABLED=true` 后，现有常驻后台运行器每个周期会附带执行天猫增量同步；所有凭据只能写入被 Git 忽略的本机 `.env`。
...
9. 派件中、已签收无退回记录、拦截失败或订单进入 `MANUAL_PROCESSING` 时，按客户档案“归属业务员”幂等生成 `ERP_CREATE_MANUAL_TODO`；发给业务员的事项包含店铺、平台订单号、发货运单和标准化物流状态，不包含售后单号，也不复制快递轨迹中的电话、地址等原文；
...
1. 按同步游标增量读取指定拼多多店铺的状态 `2/3/10` 售后；已开启的平台再独立读取天猫、淘宝、1688、京东、抖音售后记录。天猫只有在接入开关、水位和当轮同步成功条件满足时进入模块 1/2/3，模块 1/2 还必须属于退款子账号白名单；其余非拼多多平台不进入后续动作链。京东 `serviceAndRefund` 返回的纯服务单没有退款对象，明确跳过；一旦存在退款对象但金额缺失或异常，仍失败关闭；
...
12. 根据“人工待办”页面的数据库发布开关处理管理系统待办；未保存网页设置时沿用 `ERP_TODO_PUBLISH_ENABLED`。默认关闭，关闭时模块 1/2/3 仍生成本地待办但不会发送；仍须 `ERP_WRITE_ENABLED` 和登录凭据可用。开关保存后无需重启，每条提交前再次核查；
13. 分平台、分模块处理退款动作。拼多多模块 1/2 分别需要 `MODULE1_PDD_REFUND_EXECUTION_ENABLED=true` / `MODULE2_PDD_REFUND_EXECUTION_ENABLED=true`，并受 `PDD_WRITE_ENABLED` 保护；天猫模块 1/2 分别需要 `MODULE1_TMALL_REFUND_EXECUTION_ENABLED=true` / `MODULE2_TMALL_REFUND_EXECUTION_ENABLED=true`，并受 `TMALL_WRITE_ENABLED` 和店铺白名单保护。
...
快递 100 不再跟随 60 秒后台周期重复查询同一运单；同一周期内多笔售后共用同一快递公司和运单号时也只查询一次。正常取得轨迹后默认 5 分钟再刷新；夜间退款闸门会把下次检查时间直接调度到次日 09:00。技术故障仍按 5、10、20、30 分钟递增重试并计入运行失败；“查询无结果”等正常无轨迹结果单独计数，不再把整轮任务判为系统失败。连续 6 次无轨迹后会停止自动查询、冻结退款、取消待发送拦截通知，并保留本地人工核对记录，不向业务员创建或发送此类待办；发送阶段同时取消遗留的同类待发送任务；已有失败次数达到阈值的记录也会直接分流而不再请求一次接口。完整规则、恢复与回退方式见[快递100“无轨迹”分流与退款冻结](docs/logistics-no-trace-routing.md)。平台退款真正执行前仍会复查当前是否在 09:00–21:00，并强制进行一次不受退避时间限制的实时查询，避免使用夜间或缓存轨迹放款。
...
- 天猫退款写入失败或结果未知：动作记为失败并保留请求诊断，不自动重试不可逆请求；先在天猫后台核对该退款单最终状态，确认未退款后再人工决定是否重新入队。适家始终停在人工审核，不会因其他五店已配置而越过白名单；
...
- ERP 归属查询失败：保留“ERP 查询失败·待重试”，5 分钟后进入优先重查名额；正常空列表显示“ERP 未查到客户”，不当成接口故障。实际等待还取决于批次与到期失败单数量；该独立阶段不会阻断后续拦截安全流程；
- ERP 人工待办发布关闭：本地任务保留为 `PENDING`；网页开启且 ERP 总闸允许后下个周期继续。登录或查重期间关闭、尚未提交的任务会退回待发送且不消耗重试次数；已提交的不撤回。发布失败仍按原查重与重试规则处理；
...
2026-09-11 已补充发送后自动核验，正常匹配的消息自动回写成功，识别不清仍保留防重暂停。需要桌面OCR额外依赖，安装及独立发布说明见[企业微信发送后自动核验](docs/desktop-message-receipt-20260911.md)。

2026-09-11 晚间修复小窗口及窗口边框导致标题截断、正确群名被拒绝的问题；确定未输入时的前台变化按 60 秒冷却自动重试，输入后仍禁止自动重发。已完成真实业务发送并自动核验成功。部署与恢复见[企微窗口尺寸适配修复](docs/wecom-title-window-20260911.md)。

2026-09-12 修复“切到企微后不搜索群”：直接检查搜索框焦点边框，取消不可靠的整窗变化阈值；未输入时的搜索超时可冷却后自动重试，实际间隔取决于后台周期。兼容切换期间空前台句柄，保留输入后防重。已完成真实业务发送验收，见[企微搜索焦点修复](docs/wecom-search-focus-20260912.md)。
...
在途拦截页的“目标快递群”与桌面发送器共用同一标准化映射：天猫等平台返回“圆通速递”“极兔速递”等名称时，页面会先转换成快递100标准代码，再匹配数字物流公司 ID 配置。只有发送器实际无法解析群名时，页面才显示“未配置快递群”，避免出现消息已发送但页面仍误报未配置。
...
桌面文字发送器只在物流预检通过且确有待拦截任务时控制桌面；空闲周期不会激活企业微信或发送任何键盘输入。通知以“标准化快递公司代码 + 标准化发货运单号”为唯一投递组：同一合并包裹即使关联多个平台订单或售后单，也只向快递群发送一次；开始输入前会把该运单全部待通知任务一起声明为运行中，确认发送后一起转为成功并推进到 `INTERCEPT_PUSHED`。以后若同一运单新增售后任务，发送器会复用已有成功投递记录直接完成新任务，不再重复发群。开始发送时，程序按允许的 `WXWork.exe` 进程精确枚举可见窗口，拒绝安全验证窗口和多个同尺寸主窗口，并仅在尚未输入消息的2秒激活阶段有限重试，将唯一最大的企业微信主窗口切到前台；随后使用 Ctrl+1 回消息页、Ctrl+F 搜索完整群名、检测搜索画面变化、进入群聊、再次检查前台进程与验证窗口、输入消息、记录账本、按 Enter 发送并检测聊天画面变化。开始输入后、按下发送键之前不会再争抢焦点，任何前台切换都会失败关闭；按下发送键后，使用本地文字识别连续核对完整目标群名、空输入框、完整匹配的右侧消息气泡和无失败/等待标记，至少三次观测跨越两秒后才确认成功；不再仅凭画面变化确认，也不再无条件要求人工确认。确认发送成功后会尝试恢复操作者发送前使用的窗口；恢复失败不影响已经确认的发送结果。若发送结果不明，则保留企业微信在前台供人工核验，不自动切回或盲目重发。文字写入检测仅截取中间聊天输入框，避免两侧固定栏把短消息的画面变化稀释；发送后检测覆盖输入框和消息区，用于确认输入框清空及新消息气泡出现。底层 `SendInput` 使用 Windows SDK 完整 `INPUT` 联合体尺寸，兼容 32/64 位 Windows；系统拒绝或未完整接收任一组按键时立即在输入消息前暂停。任意阶段按 ESC 都停止；安全验证、登录验证、发送前的前台切换或画面未变化都会失败关闭。

ESC 中止只读取 Windows 返回的“当前按下”高位；“自上次查询以来曾出现按键事件”的低位可能由历史操作或其他桌面软件留下，必须忽略，不能据此把未按 ESC 的操作者误判为主动中止。发送前暂停仍会写入 `PausedBeforePaste`，确认没有输入消息后使用既有恢复命令继续。

2026-09-05 补充焦点与异常保护：激活阶段要求选定的同一个企业微信主窗口连续保持前台至少 300 毫秒，仍受 2 秒总时限约束；每组搜索、文字、换行和发送按键前再次校验进程和窗口句柄。截图前后都必须保持目标窗口在前台，避免将其他窗口覆盖后的屏幕变化算作发送成功。已经进入 `PasteStarted` / `SendPressed` 后，截图、键盘等异常均保留原进度，并将错误追加到账本和数据库，不能降级成可重试的 `PausedBeforePaste`。该保护只能减少失焦期间的继续输入和误判，不能让桌面发送在操作者同时切换窗口时可靠运行。若出现“消息输入后未检测到聊天区域变化”，表示输入校验失败且尚无发送确认；先核对目标群消息及草稿，再按下方恢复流程处理，不能直接清账本或将任务改成成功。修改部署前备份 `.runtime/module1-worker.log`，按 `module1-worker.ps1 -Action Stop` 等待本轮结束，再 `Start` 加载新代码；重启不会解除尚未核验任务。
...
5. 型号、颜色、数量任一不一致时登记“验货异常”，并按 ERP 归属业务员幂等生成 `ERP_CREATE_MANUAL_TODO`；未退款订单冻结平台退款，已退款订单使用独立幂等标识补发“退款后退货异常申诉”待办，明确要求业务员向平台发起申诉并跟进少退、错退或未收到商品的申诉结果；待办会列出平台申请、ERP 实收、退货单号和具体缺少/多出明细；
...
8. 退款动作使用售后单级幂等键，执行前再次核对仓库验货事实；拼多多按店铺和订单隔离：当轮接口整体失败的店铺停止执行，正常窗口已提交的店铺可处理无隔离问题的订单，不因另一店/另一笔历史单异常停止。隔离订单已有的旧任务也不得执行。平台已经退款时只补齐本地审计，不再次调用退款接口；状态不明或不属于可退款状态时失败关闭，保留错误供人工处理。

退款后追货是独立于退款动作的持续核对链路：缺少买家退货运单时显示“退款后待退货运单”，已有运单但 ERP 客户退货单和退货暂存列表都未找到时显示“退款后待仓库收货”；ERP 收货后，实收型号、颜色、数量完全一致则显示“退款后验收一致”，少退、错退或数量不符则通知归属业务员向平台发起申诉并跟进结果。若验货异常待办创建后平台才完成退款，系统会以独立幂等标识补发申诉待办，不会被原待办拦截；该链路覆盖普通退款成功和极速退款，不会生成第二次退款任务。

天猫平台已成功的退货退款另有严格的ERP补单闭环：同一父订单的一笔或多笔退款必须与全部平台子单、同一退货运单、ERP退款详情、本父订单唯一RC/TH商品及可选税点行、唯一原收款、逐笔SK退款流水和累计应收完整对应；同客户其他已结清历史订单不参与当前组，但混入当前订单标识的记录仍阻断，任一当前组证据缺失或多余即不写。每个Worker周期最多新补一笔，下一笔重新核验余额；已存在流水只对账，超时或结果不明不重发同一笔。该分支不会再次调用平台退款，完整边界见[天猫模块1：暂存认领与ERP补单](docs/tmall-module1-return-claim.md#2026-09-23模块2退货退款成功后的分组erp补单)。
...
自动化边界与恢复方式和七店售后增量同步一致：网络、鉴权、无法定位的列表记录及入库/台账失败会回滚当前窗口，不推进 `pdd_sync_cursors`；修复后从最后成功游标向前重叠 5 分钟幂等续传。具有有效订单号和售后号的映射错误改为持久化隔离并重查，正常单继续入库，异常未解决仍保留同步告警和资金保护。拼多多 `created_time`、`updated_time` 兼容 Unix 秒时间戳及 `YYYY-MM-DD HH:MM:SS` / ISO 日期文字；无法识别的时间记入该笔异常台账，不填造日期或丢弃记录。原因分类只影响看板归因，不参与拦截、退款或 ERP 动作判定。
...
- 左侧“运行监控”页面每 15 秒刷新模块 1/2/3 常驻进程、最近完整周期、各处理阶段和当前启用范围内的企微拦截任务队列；除发送前失败任务的受控重新入队外，页面不触发退款、发送消息或控制企业微信。`PausedBeforePaste` 可由操作者确认后重试，`PasteStarted` / `SendPressed` 必须先到群内人工核验；
- 左侧“接入能力”页面按平台和店铺展示售后同步、售后归因、退款统计、退款权限、模块 1 拦截退款/退回平账、模块 2 验货退款及模块 3 未发货平账；“未开启”表示已有接入但开关或授权不完整，“未接入”表示当前平台代码尚不支持该功能，鼠标悬停可查看缺失原因；状态实时取自本机环境开关、凭证是否存在及数据库店铺登记，接口不返回凭证明文；
...
- `GET /api/v1/monitor/capabilities`：只读返回六个平台及当前配置店铺的功能矩阵、开通原因、售后记录数量和最近记录时间；只报告凭证是否齐全，不返回任何密钥、SessionKey 或 AccessToken。
...
售后订单、在途拦截和人工待办页面都只读，不会直接调用拼多多退款、企微发送或 ERP 写接口。在途拦截页没有“发送”或“退款”按钮，真实外部动作仍只能由后台运行器在对应总开关打开后执行。运行监控唯一的受控写操作是把明确尚未输入消息的 `PausedBeforePaste` 任务重新入队；它不直接按键发送，也不能恢复结果不明任务。运行监控显示“未运行”或“周期长时间未完成”时，先用 `& .\scripts\module1-worker.ps1 -Action Status` 核对，再按 `Stop` / `Start` 安全恢复；它不会从网页强制重启进程。仓库验货页面会写入本地 `warehouse_return_*` 表和对应售后状态，但不会触发任何外部写操作。数据库暂未保存买家昵称时，详情明确显示“平台未返回”，不会虚构客户信息。
...
## 2026-09-08 自动化可靠性修复（第一批）

上线前须执行数据库迁移 `20260908_0020`，新增公平轮询进度和 1688 异常单重查台账。模块 1 待办、模块 2 退货匹配、模块 3 ERP 补单不再让固定页旧任务反复占位；拼多多自动退款增加最新平台事实核验，失败任务只读核实后收尾或转人工，不盲目重试。1688 异常单持久化隔离后允许正常窗口继续推进，异常仍可见并单独重查。

本批未实现 ERP 自动认领或可靠桌面发送内容核验，也未恢复人工待办自动发布。旧章节中的开关启用时间是历史记录，当前运行以本机配置为准。迁移、频率、写入边界、恢复步骤和剩余事项见 [自动化可靠性修复说明](docs/automation-reliability-20260908.md)。

## 2026-09-08 状态展示修复（第二批）

售后列表、详情、拦截提示和顶部“平台已退款”统计统一以平台成功事实为准，不再把仓库验货通过当作退款完成。缺实付金额显示“缺买家实付”；ERP 归属区分历史未查询、等待查询、同步未开启、未接入、未找到和查询失败，并在详情展示原因及最近查询时间。浏览页面只读本地归属缓存，外部查询继续由原后台分批同步负责。

本批没有新增外部写入或修改自动化频率，无数据库迁移。完整口径、配置边界与验证说明见 [售后状态展示说明](docs/record-status-20260908.md)。

## 2026-09-08 历史资料补查（第三批）

新增独立的历史资料隔离补查入口，默认只读预演。拼多多仅补买家实付，不补商家应收或重新分类流程；天猫只补既有自动化水位之前的客户归属缓存，不修改上线水位。命令不创建动作任务，不调用平台退款、企微发送、ERP 认领或补单。

正式补写前备份，按明确的本地主键上限分批处理；失败记录独立保存、24小时后才可在再次手工运行时重查，连续3次失败停止本批。没有新增定时任务。命令、安全筛选、并发保护和未解决的天猫金额查询问题见 [历史资料补查说明](docs/historical-supplement-20260908.md)。

## 2026-09-08 天猫历史客户归属核查（第四批）

35条未找到客户归属的售后对应27笔订单，逐笔核对平台退款、发货事实及 ERP 退款记录：均已退款，平台未确认发货，ERP 未关联销售单和客户。未猜测业务员或标记退回平账完成。其中3条遗漏的成功状态，通过点名历史记录的 `tmall_status` 补查入口仅补回本地三个状态字段；不调用退款、不覆盖金额、不创建动作任务，不改变自动化水位。

使用边界、证据限制、备份及剩余发货状态映射风险见 [历史资料补查说明](docs/historical-supplement-20260908.md)。

## 2026-09-08 天猫发货事实保护（第五批）

先升级数据库至 `20260908_0021` 再运行新版本：新增发货状态 `UNKNOWN`，交易关闭不再默认未发货，已发货/签收事实不被后续退款覆盖。模块3新任务和 ERP 回填增加保护；现有拼多多 ERP 自动补单器不处理天猫旧任务，天猫自动补单仍需单独适配验收。

本批1127条旧天猫误标仅修正为待核实，金额、财务结果、业务流程与任务不变；没有重放历史退款或补单。命令、备份、恢复和验证见 [天猫发货状态保护说明](docs/tmall-shipping-guard-20260908.md)。

## 2026-09-08 天猫历史退款事实补查（第六批）

对剩余38条历史天猫退款待核实记录逐一读取平台详情，新增点名隔离入口 `tmall_refund_facts`。只补已验证的退款成功及对应退款子单的发货事实；父单完成、其他子单发货不作为本子单签收证据。部分退款不重新进入拦截，金额、业务员、ERP 闭环和动作任务保持原样。

默认预演，无新增定时任务、配置变更或服务重启。命令、安全条件、备份和结果见 [天猫历史退款事实补查说明](docs/tmall-refund-facts-20260908.md)。

## 2026-09-08 人工待办网页开关与 ERP 闭环复核

“人工待办”顶部新增“自动发布给业务员”开关，统一控制模块1/2/3待办发布。关闭保留本地队列，开启需确认积压数量；数据库持久保存、后台实时读取，无需修改 `.env` 或重启。未保存网页设置时使用 `ERP_TODO_PUBLISH_ENABLED` 初始值，保存后数据库值优先；因此前文有关仅修改该环境变量来开关/重启的步骤属于旧版操作，现以网页开关为准。`ERP_WRITE_ENABLED` 总闸和凭据要求始终保留。

升级需执行迁移 `20260908_0022`。本次维持关闭，不发送36条积压待办。开关写接口仅接受本机同源访问，具备版本冲突保护及操作记录；发送中已提交的任务不可撤回。人工待办页面不再完全只读，但开关只保存本地控制状态，不直接调用外部发布接口。模块2筛选同步修复。

ERP 待匹配42笔按未找到、明细不一致、应收未归零、暂存待认领分组，并各抽一笔只读复核，没有认领、补单或抹平差额。边界、频率、升级恢复及验证见 [人工待办开关说明](docs/manual-todo-switch-20260908.md)。
