...
模块 4 复用各平台已经同步入库的售后事实，不新增另一个爬取任务。每条售后进入同步窗口时，同步器自动读取平台、店铺、SKU、商品名、退款件数、平台原始原因、买家留言及平台申请/更新时间，并在同一数据库事务中幂等保存。`20260902_0013` 迁移会对已有历史售后的 `reason_category` 自动回填；历史记录没有平台时间时，看板明确回退为首次入库时间。

工作台左侧“售后归因”页面提供：

- 按“平台 → 店铺”两级联动、平台申请时间、型号/SKU 和原因大类筛选；
...
- `GET /api/v1/attribution/overview`：模块 4 售后归因摘要、原因构成、型号排名、店铺分布和选中型号下钻；支持 `platform`、`shop_id`、`started_on`、`ended_on`、`model_keyword`、`reason_category` 和 `focus_model`。
