...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_warehouse_return_inspection",
        "warehouse_return_records",
    ...
    op.create_table(
        ...
        sa.Column("color", sa.String(length=50), server_default="", nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index("idx_warehouse_return_inspection", table_name="warehouse_return_records")
    op.drop_index("idx_warehouse_return_destination", table_name="warehouse_return_records")
    op.drop_index("idx_warehouse_return_after_sales", table_name="warehouse_return_records")
    op.drop_table("warehouse_return_records")

    op.execute(
        "UPDATE aftersales_orders SET workflow_status='RETURN_WAITING_SCAN' "
        "WHERE workflow_status IN ('RETURN_RECEIVED_STAGED','RETURN_RECEIVED_ASSIGNED')"
    )
    op.alter_column(
        "aftersales_orders",
        "workflow_status",
